{
  ...
  ldapCertDomain = config.networking.fqdn;
in {
  networking.firewall.allowedTCPPorts = [389];
  security.acme.certs."${ldapCertDomain}".reloadServices = ["openldap"];
    ...
    settings = {
      ...
      children = {
        ...
        "olcDatabase={1}mdb".attrs = {
          ...
          olcAccess = [
            ''
              {0}to dn.base="${dbSuffix}" by anonymous read
                by * break''
            ''
            ...
            ''
              {2}to attrs=userPassword
                by group/organizationalRole/roleOccupant="${ldapRootDN}" write
                by * auth''
            ''
              {3} to attrs=@posixAccount
                ...
                by * break''
            ''
              {4}to dn.subtree="ou=Realms,${dbSuffix}"
                by dn.exact="cn=kdc-admin,ou=Service,${dbSuffix}" write
                by dn.exact="cn=kdc-read,ou=Service,${dbSuffix}" read
                by group/organizationalRole/roleOccupant="cn=kdc-admin,ou=Service,${dbSuffix}" write
                by group/organizationalRole/roleOccupant="cn=kdc-read,ou=Service,${dbSuffix}" read
                by * none''
            ''
            ...
            ''
              {7}to attrs=krbLastSuccessfulAuth,krbLastFailedAuth,krbLoginFailedCount
                by dn.exact="cn=kdc-admin,ou=Service,${dbSuffix}" write
                by dn.exact="cn=kdc-read,ou=Service,${dbSuffix}" write
                by group/organizationalRole/roleOccupant="cn=kdc-admin,ou=Service,${dbSuffix}" write
                by group/organizationalRole/roleOccupant="cn=kdc-read,ou=Service,${dbSuffix}" write
                by * none''
            ''
        ...
        "olcDatabase={2}monitor".attrs = {
          ...
          olcAccess = ["{0}to * by group/organizationalRole/roleOccupant=\"${ldapRootDN}\" read"];
        };
      }; # children
    }; # settings
  }; # services.openldap
